    ...
    <div className= "flex flex-col p-2 w-2xl h-auto gap-2 rounded-2xl text-white" id = "add-task">
      <div className="flex flex-col gap-10">
        ...
        <ul className="list-disc list-inside text-2xl">
            {tasks.map((userTask) => (
                <li className="flex w-fit items-center hover:bg-zinc-700 hover:rounded-md hover:p-2  hover: scale-105transition-transform gap-3">{userTask.name} 
                        <button onClick = {() => RemoveTask(userTask.id)} className="w-fit text-zinc-400 hover:scale-105 transition-transform hover:cursor-pointer rounded-lg hover:bg-zinc-500">×</button>
                </li>
            ))}
